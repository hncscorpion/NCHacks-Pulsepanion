...
```{r}
reshape_sleep_act <- function(df) {
  # Column names
  old <- c("Date", "Description", "Is Important", "Time", "Created By")
  new <- c("date", "sleep_act_desc", "sleep_act_imp", "sleep_act_time", "sleep_act_cb")
  ...
  mutate(
    date = mdy(date),
    sleep_act_imp = as.factor(sleep_act_imp),
    sleep_act_time = parse_time(sleep_act_time),
    sleep_act_cb = as.factor(sleep_act_cb)
  )
...
```{r}
# Apply changes
set1_dfs[["Sleep Activities"]] <- reshape_sleep_act(set1_dfs[["Sleep Activities"]])
set2_dfs[["Sleep Activities"]] <- reshape_sleep_act(set2_dfs[["Sleep Activities"]])
set3_dfs[["Sleep Activities"]] <- reshape_sleep_act(set3_dfs[["Sleep Activities"]])
...
- The following columns contain data on activities during the day/night that Patient X engaged in, or observations that a clinician made of them. Missing values for these columns will be flagged.
  ...
  - sleep_act_desc -> description of a sleep-related event that occurred
...
all_joined <- all_joined %>%
  mutate(
    ...
    sleep_desc_missing = is.na(sleep_act_desc),
    sleep_imp_missing  = is.na(sleep_act_imp),
    sleep_time_missing = is.na(sleep_act_time),
...
all_joined <- all_joined %>%
  mutate(
    ...
    sleep_act_imp_clean = ifelse(is.na(sleep_act_imp), "Unknown", as.character(sleep_act_imp)),
    ...
    sleep_act_desc_clean = ifelse(is.na(sleep_act_desc), "No sleep-related activity recorded", sleep_act_desc),
